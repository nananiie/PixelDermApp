...
const PixelDermApp = () => {
  ...
  // Generate and share a PDF of the current analysis result.
  const handleShareAnalysis = async () => {
    ...
    const riskClass = riskInfo.label === 'Low' ? 'risk-low' : riskInfo.label === 'Moderate' ? 'risk-moderate' : 'risk-high';

    const baselineSection = baseline ? `
      <div class="section">
        <div class="section-title">Baseline Comparison</div>
        <div class="row"><span class="label">Status</span><span class="value ${riskClass}">${recommendation.status}</span></div>
        <div class="row">
          <span class="label">Spots</span>
          <span class="value">${features.spotCount} <span class="delta ${spotDelta! > 0 ? 'worse' : 'better'}">(${spotDelta! > 0 ? '+' : ''}${spotDelta} vs baseline ${baseline.spotCount})</span></span>
        </div>
        <div class="row">
          <span class="label">Texture</span>
          <span class="value">${Math.min(features.textureScore * 100, 100).toFixed(1)}% <span class="delta">(baseline ${Math.min(baseline.textureScore * 100, 100).toFixed(1)}%)</span></span>
        </div>
        <div class="row">
          <span class="label">Pigmentation</span>
          <span class="value">${(features.pigmentation * 100).toFixed(1)}% <span class="delta">(baseline ${(baseline.pigmentation * 100).toFixed(1)}%)</span></span>
        </div>
      </div>` : '';

    const uvSection = uvDamage ? `
      <div class="section">
        <div class="section-title">UV Damage Assessment</div>
        <div class="row"><span class="label">UV Damage Level</span><span class="value">${uvDamage.level}</span></div>
        <div class="recommendation">${uvDamage.advice}</div>
      </div>` : '';

    const aiSection = geminiRecommendation ? `
      <div class="section">
        <div class="section-title">AI Recommendation</div>
        <div class="recommendation ai">${geminiRecommendation}</div>
      </div>` : '';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: Arial, sans-serif; color: #333333; background: #fff; padding: 28px; font-size: 13px; }
    .header { background: #91AFC2; color: #fff; padding: 22px 24px; border-radius: 10px; margin-bottom: 22px; }
    .header h1 { font-size: 20px; font-weight: bold; margin-bottom: 4px; }
    .header p { font-size: 11px; opacity: 0.88; }
    .section { border: 1px solid #E0EAEF; border-radius: 10px; overflow: hidden; margin-bottom: 18px; }
    .section-title { background: #D1E0E8; padding: 9px 16px; font-weight: bold; font-size: 12px; color: #5A7D8F; text-transform: uppercase; letter-spacing: 0.5px; }
    .row { display: flex; justify-content: space-between; align-items: center; padding: 9px 16px; border-bottom: 1px solid #f0f4f7; }
    .row:last-child { border-bottom: none; }
    .label { color: #7D8F99; font-size: 12px; }
    .value { font-weight: bold; font-size: 13px; color: #333; }
    .delta { font-weight: normal; font-size: 11px; color: #7D8F99; }
    .worse { color: #F44336; }
    .better { color: #4CAF50; }
    .risk-low { color: #4CAF50; }
    .risk-moderate { color: #FF9800; }
    .risk-high { color: #F44336; }
    .recommendation { padding: 14px 16px; font-size: 13px; line-height: 1.65; color: #444; background: #F8FBFC; }
    .recommendation.ai { border-left: 3px solid #5A7D8F; background: #EEF4F8; }
    .disclaimer { margin-top: 20px; padding: 12px 16px; background: #FFF8E1; border-left: 4px solid #FF9800; border-radius: 6px; font-size: 11px; color: #7D6000; line-height: 1.6; }
    .footer { text-align: center; margin-top: 22px; font-size: 10px; color: #AAAAAA; border-top: 1px solid #E0EAEF; padding-top: 14px; }
  </style>
</head>
<body>
  <div class="header">
    <h1>PixelDerm Skin Analysis Report</h1>
    <p>Generated: ${scanDate}</p>
  </div>

  <div class="section">
    <div class="section-title">Patient Information</div>
    <div class="row"><span class="label">Name</span><span class="value">${activeProfile?.name ?? '—'}</span></div>
    <div class="row"><span class="label">Age</span><span class="value">${activeProfile?.age ?? '—'}</span></div>
    <div class="row"><span class="label">Sex</span><span class="value">${activeProfile?.sex ?? '—'}</span></div>
    <div class="row"><span class="label">Skin Type</span><span class="value">${activeProfile?.skinType ?? '—'}</span></div>
    <div class="row"><span class="label">Area Analyzed</span><span class="value">${activePart}</span></div>
  </div>

  <div class="section">
    <div class="section-title">Measured Metrics</div>
    <div class="row"><span class="label">Skin Score</span><span class="value ${riskClass}">${skinScore}%</span></div>
    <div class="row"><span class="label">Risk Level</span><span class="value ${riskClass}">${riskInfo.label}</span></div>
    <div class="row"><span class="label">Spots Detected</span><span class="value">${features.spotCount}</span></div>
    <div class="row"><span class="label">Texture Score</span><span class="value">${Math.min(features.textureScore * 100, 100).toFixed(1)}%</span></div>
    <div class="row"><span class="label">Pigmentation</span><span class="value">${(features.pigmentation * 100).toFixed(1)}%</span></div>
  </div>
  ...
  <div class="section">
    ...
    <div class="recommendation">${recommendation.advice}</div>
  </div>
  ...
  <div class="disclaimer">
    ...
    Always consult a qualified dermatologist for professional evaluation and treatment.
  </div>

  <div class="footer">Generated by PixelDerm &nbsp;•&nbsp; ${scanDate}</div>
</body>
</html>`;
    ...
    try {
      const pdf = await RNHTMLtoPDF.convert({
        html,
        fileName: `PixelDerm_Report_${Date.now()}`,
        directory: 'Documents',
      });
      ...
      await RNShare.open({
        ...
        title: 'PixelDerm Analysis Report',
        failOnCancel: false,
      });
    } catch (e: any) {
